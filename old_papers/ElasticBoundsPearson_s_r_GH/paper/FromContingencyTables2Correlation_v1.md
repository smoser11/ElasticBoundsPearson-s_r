...
- A common but problematic assumption is that ordinal variables can be  treated as interval-scaled, allowing use of parametric statistics like  Pearson’s $r$.  There are actually two problems here:
	- 1. _Value assignment_: Which values or scores  (monotic) should be assigned to categories.  This has been discussed in CITES.  The general consensus is that this doesn't matter much [DOESN'T MATTER FOR WHAT?].  Indeed drawing on the psychophysical scaling literature (CITE M. Lodge?) ....  This is not a discussion we contribute to.
	- 2. _Bounded measurement_ introduce kinds of non-linearities.  For example in the latent variable interpretation of Ordinal Variables, the underlying latent variable is censored.
...
- And many more: $\eta$, $\eta^2$ (coefficients of non-linear association CITES)
...
- We generate random contingency tables via permutation-based randomization (@fish35-design; @ghos20-exact), preserving marginal distributions.  These are non-parametric  methods of null-hypothesis significance testing.   The core idea is to evaluate the likelihood of observing a test statistic under the null hypothesis, by randomly shuffling the labels or group assignments of the data many times.
