...
\subsubsection{\texorpdfstring{\textbf{Same number of categories, but
\(|r_{\min}|\ne r_{\max}\)} (concrete
numbers)}{Same number of categories, but \textbar r\_\{\textbackslash min\}\textbar\textbackslash ne r\_\{\textbackslash max\} (concrete numbers)}}\label{same-number-of-categories-but-r_minne-r_max-concrete-numbers}
...
\subsubsection{\texorpdfstring{\textbf{Different numbers of categories,
yet \(|r_{\min}|=r_{\max}\)} (concrete
numbers)}{Different numbers of categories, yet \textbar r\_\{\textbackslash min\}\textbar=r\_\{\textbackslash max\} (concrete numbers)}}\label{different-numbers-of-categories-yet-r_minr_max-concrete-numbers}
...
\subsubsection{\texorpdfstring{\textbf{Spearman invariance (same data as
in
B)}}{Spearman invariance (same data as in B)}}\label{spearman-invariance-same-data-as-in-b}
...
\subsubsection{Setup (permutation extremes for a single
...
\subsubsection{Pair-specific necessary \& sufficient
...
\subsubsection{\texorpdfstring{Universal condition (independent of
\(X\))}{Universal condition (independent of X)}}\label{universal-condition-independent-of-x}
...
The benchmarks for correlation effect size proposed by Cohen et al.
(\citeproc{ref-cohe02-applied}{2002}) --- small (\(r \approx 0.10\)),
moderate (\(r \approx 0.30\)), and large (\(r \approx 0.50\)) --- have
become standard reference points in the social sciences. Their implicit
...
The comparability problem extends beyond single-study effect size
interpretation to any research design that places correlations from
different variable pairs, studies, samples, or time points in relation
to one another. Cumulative social science --- through meta-analysis,
cross-national comparison, and longitudinal trend analysis --- rests on
the assumption that reported correlations are commensurable. Elastic
bounds undermine this assumption.

The most immediate source of non-comparability is variation in the
number of response categories. The Monte Carlo results of
Section~\ref{sec-MCsim} show that \(C_1\) increases systematically with
\(K\): pairs involving variables with more categories tend to have
broader attainable ranges and thus less constrained correlations. A
study measuring an association on 5-point scales will, on average,
report a smaller correlation than a study measuring the same association
on 7-point scales --- not because the underlying relationship is weaker,
but because the attainable range is narrower. Any meta-analysis that
pools raw \(r\) values across studies differing in scale length will
conflate measurement constraints with substantive variation in effect
magnitude.
