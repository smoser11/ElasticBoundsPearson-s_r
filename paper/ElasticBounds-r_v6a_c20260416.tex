...
This article proceeds as follows. \Cref{sec-background}
...
coupling framework that underlies our bounds. \Cref{sec-theory}
...
signs of extreme correlations. \Cref{sec-metrics} introduces
metrics of constraint --- scalar summaries of how severely marginal
distributions restrict the attainable range of \(r\).
\Cref{sec-MCsim} presents Monte Carlo validation across orcat
pairs with 4 to 11 categories, characterising how marginal shape drives
constraint. \Cref{sec-bes} illustrates the findings using 7,503
variable pairs from the British Election Study 2019.
\Cref{sec-implications} draws out applied consequences for
...
In our simulation study (\Cref{sec-MCsim}), we compute all three
...
\Cref{sec-theory} show --- concentrated (skewed) marginals are
...
relationship is examined empirically in \Cref{sec-MCsim} and
\Cref{sec-bes}.
...
defined in \Cref{sec-metrics}. Further details of the simulation
...
\Cref{sec-bes} provides the empirical complement, characterising
...
\Cref{sec-metrics} introduced Shannon entropy
...
The theoretical results in \Cref{sec-theory} establish precisely
...
metrics in \Cref{sec-metrics} all have negligible predictive
...
constructions of \Cref{sec-theory}. We do not claim that the BES
...
confirming the simulation finding of \Cref{sec-MCsim}: the
...
BES example introduced in \Cref{sec-theory} --- where
...
The rescaled correlation \(r^*\) (\Cref{sec-implications-resc})
...
\Cref{sec-background}). It also does not address the asymmetry
problem directly: when \(|r_{\min}| \neq r_{\max}\), the rescaled zero
is not the same as \(r_{\text{obs}} = 0\). Adjusted null distributions
via permutation testing (\Cref{sec-implications-hyp}) provide a
...
\item
  ...
  \Cref{sec-matrices} above for implications for correlation
  matrices, FA, and SEM
\end{itemize}
